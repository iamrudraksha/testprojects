...
extension String {
    ...
    */
    ...
    :param: withString the string to add instead of target
...
extension NSDate {
    ...
    */
    func timeToNow() -> String {
        ...
        var days = Int(floor(timeInterval / (3600 * 24)))
        var hours = Int(floor((timeInterval % (3600 * 24)) / 3600))
        var minutes = Int(floor((timeInterval % 3600) / 60))
    ...
    */
    class func parseFullDate(var string: String) -> NSDate? {
        string = string.substringToIndex(advance(string.startIndex, FullDateParsers.dateStringLength))
...
:param: callback the callback to invoke after 'delay' seconds
*/
