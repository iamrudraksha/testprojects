...
class IdeasListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate,
    ...
    */
    func loadData() {
        ...
        IdeasDataSource.sharedInstance.getIdeas(callback: { (var list: [Idea]) -> () in
            ...
            if self.listType == .Random {
                sorted = list.sorted({ (i1: Idea, i2: Idea) -> Bool in
                    return Bool.random()
                })
            }
            // Sort by date descending
            else {
                sorted = list.sorted({ $0.created.compare($1.created) == NSComparisonResult.OrderedDescending })
            }
    ...
    */
    ...
    :param: needToShow flag: true - need to show search field, false - hide
    */
    func showSearch(needToShow: Bool) {
        needToShowSearch = needToShow
        if needToShow {
            searchTextField = self.navigationController?.setNavigationForSearching("Search ideas", delegate: self)
    ...
    */
    ...
    :param: referenceIdea the reference idea
    ...
    */
    ...
    :param: section   the section index (zero)
    ...
    */
    ...
    :param: tableView the tableView
    :param: indexPath the indexPath that is selected
    */
