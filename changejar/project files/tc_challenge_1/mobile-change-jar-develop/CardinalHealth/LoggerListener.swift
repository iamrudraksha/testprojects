...
class LoggerListener: NSObject {
    ...
    */
    ...
    func receivedLogNotication(notification: NSNotification) {
        if self.isListening {
            if let object: AnyObject = notification.object {
                Logger.logDebug(object.description)
            }
            else {
                Logger.logDebug("<empty info notification>")
            }
        }
    }
    ...
    func addObservers() {
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "receivedLogNotication:",
