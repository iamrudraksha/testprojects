...
* 1.2:
* - text font is reduced (due to "ideas" to "actions" replacement the text takes more space)
*/
class HomeViewController: UIViewController {
    ...
    */
    override func viewDidAppear(animated: Bool) {
        ...
        if isFirstTime(kHomeOverlayShown) { // Check if Home Screen is first time opened
            updateNotFirstTime(kHomeOverlayShown)
            self.showOverlay()
        }
    }
    ...
    */
    func initNavigation() {
        ...
        // Add widget
        let widget = NotificationsWidget()
        widget.number = 0
    ...
    */
    func showOverlay() {
        ...
        if let vc = create(OverlayViewController.self) {
            ...
            _ = navigationController!.createSnapshort()
            ...
            let data3 = HomeOverlay(text: "What's your actions? Share & discuss with others",
    ...
    - parameter localizedPrefix: the prefix for localized string key
    ...
    */
    func getAttributedString(localizedPrefix: String) -> NSMutableAttributedString {
        ...
        let range3 = NSMakeRange((str1 + str2).characters.count, str3.characters.count)
